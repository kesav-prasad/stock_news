...
async function fetchNewsForCompany(prisma, companyId, symbol, name) {
    try {
        ...
        if (isStale) {
            ...
            const refreshTask = (async () => {
                try {
                    ...
                    const results = await Promise.all(queries.map(q => parser.parseURL(`https://news.google.com/rss/search?q=${encodeURIComponent(q)}&hl=en-IN&gl=IN&ceid=IN:en`)
                        .catch(err => {
                        console.error(`[Scraper] Query failed for "${q}":`, err.message);
                        return { items: [] };
                    })));
                    const bestResult = results.reduce((prev, curr) => (curr.items.length > prev.items.length) ? curr : prev, { items: [] });
                    ...
                    for (const item of topEntries) {
                        if (!item.title || !item.link || existingUrls.has(item.link))
                            continue;
